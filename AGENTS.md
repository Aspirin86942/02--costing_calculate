...
- `产品数量统计`sheet保留现有行粒度，仅保留`本期完工数量 > 0`且`本期完工金额`非空的工单；输出三大类金额、制造费用细项金额、独立成本项金额、单位成本、勾稽状态与异常原因说明。
- `产品数量统计`sheet中的`制造费用明细项合计是否等于制造费用合计`仅校验制造费用明细，不包含独立成本项；`gb`下独立成本项为`委外加工费`，`sk`下独立成本项为`委外加工费`和`软件费用`。
- `产品数量统计`sheet中的总成本勾稽口径按管线区分：
