...
def infer_rename_map(
    ...
    *,
    ...
    if child_material_column not in normalized_columns:
        candidates = [column for column in normalized_columns if '物料编码' in column or '子件' in column]
        if candidates:
            rename_map[candidates[0]] = child_material_column

    if cost_item_column not in normalized_columns:
        candidates = [column for column in normalized_columns if '成本项目' in column or '费用项目' in column]
        if candidates:
