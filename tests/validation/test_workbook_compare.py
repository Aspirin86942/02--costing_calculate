...
from tools.validation.workbook_compare import (
    NUMERIC_COLUMNS,
    WorkbookMismatch,
    compare_workbooks,
    workbook_payloads_identical,
)
...
def test_package_fast_path_ignores_only_volatile_core_properties(tmp_path: Path) -> None:
    expected, actual = _matching_workbooks(tmp_path, [['字段'], ['合成文本']])

    assert workbook_payloads_identical(expected, actual)

    _rewrite_zip_xml(actual, 'xl/sharedStrings.xml', lambda xml: xml.replace('合成文本'.encode(), b'changed'))

    assert not workbook_payloads_identical(expected, actual)
